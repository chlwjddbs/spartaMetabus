...
namespace TheStack
{
    public class TheStack : MonoBehaviour
    {
        ...
        private const string BestScoreKey = "BestSocre";
        ...
        private void GmaeOverEffect()
        {
            int childCount = this.transform.childCount;

            for (int i = 1; i < 20; i++)
            {
                if (childCount < i) break;  //childCount i보다 작다면 브레이크를 건다. 인덱스보다 작이지기 때문

                GameObject go = transform.GetChild(childCount - i).gameObject;

                if (go.name.Equals("Rubble")) continue; //러블은 이미 바닥으로 떨어져 없어지기 때문에 처리하지 않는다.
        ...
        private bool Spawn_Block()
        {
            ...
            UIManager.Instance.UpdateSocre();
            return true;
        }
